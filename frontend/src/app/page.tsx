...
export default function Home() {
  ...
  const POLLING_URL = 'http://localhost:8100/agent_messages';
  ...
  useEffect(() => {
    const pollAgentMessages = async () => {
      try {
        ...
        const newMessages = agentMessages
          .filter((msg: any) => msg.timestamp > lastMessageTimestamp.current)
          .map((msg: any) => ({
            content: msg.message,
            isUser: false,
    ...
    const pollingInterval = setInterval(pollAgentMessages, 2000);
    pollAgentMessages();
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorMessage: Message = {
        content:
          '⚠️ Could not connect to the NakalTrade agent. Please ensure it is running.',
        isUser: false,
        agentName: 'NakalTrade',
      };
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-[#1a1a1a]">
      <div className="chat-container w-full max-w-3xl h-[90vh] max-h-[800px] bg-[#252525] rounded-xl border border-[#333] flex flex-col shadow-lg">
        <div className="chat-header p-5 bg-[#333] border-b border-[#444] rounded-t-xl text-center">
        ...
        <div className="chat-messages flex-1 p-5 overflow-y-auto">
          {messages.map((msg, index) => (
            ...
            >
              ...
              >
                {!msg.isUser && (
                  <span className="agent-name font-bold block mb-1 text-[#007aff]">
                    {msg.agentName}
                  </span>
                )}
        ...
        {isTyping && (
          <div className="typing-indicator p-3 text-sm italic text-[#aaa]">
            NakalTrade is analyzing...
          </div>
        )}
